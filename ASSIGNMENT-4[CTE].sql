...
-- the actor_award table , avoid multivalued (each column has only atomic values) , Also we can create seperate tables for the columns which contain multiple values etc 
...
-- que 10  Create a CTE to list customers who have made more than two rentals, and then join this CTE with the customer table to retrieve additional  customer details
